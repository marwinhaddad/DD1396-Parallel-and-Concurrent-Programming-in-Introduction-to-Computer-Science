...
func TestWordCountOnLoremIpsum(t *testing.T) {
	...
	if actual := WordCountTest(string(data)); !reflect.DeepEqual(actual, expected) {
		t.Errorf("expected: %v\nactual: %v", expected, actual)
	}
}

func WordCountTest(text string) map[string]int {
	ch := make(chan map[string]int)
	slices := make(chan []string, RoutinesTest-1)
	...
	for i := 0; i < RoutinesTest; i++ {
		go SliceTextTest(slices, arr)
		go WordCountSliceTest(slices, ch)
	}
	for i := 0; i < RoutinesTest; i++ {
		tempMap := <-ch
...
func SliceTextTest(slices chan []string, arr []string) {
	size := (len(arr) + RoutinesTest - 1) / RoutinesTest
...
func WordCountSliceTest(slices chan []string, ch chan map[string]int) {
